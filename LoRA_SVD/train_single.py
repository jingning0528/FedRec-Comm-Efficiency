...
class NCFTrainer:
	def __init__(self, ui_matrix, epochs, batch_size, latent_dim=32, lora_rank=8, device=None):
		self.ui_matrix = ui_matrix
		self.epochs = epochs
		...
		self.ncf = NeuralCollaborativeFiltering(
			self.ui_matrix.shape[0], self.ui_matrix.shape[1],
			self.latent_dim, lora_rank=self.lora_rank
		).to(self.device)
