...
# ── Upload size: only A + MLP (NOT B) ────────────────────────────────────────

def _upload_bits(item_num: int, latent_dim: int, rank: int) -> int:
    """
    Paper CoLR upload per client:
      - mlp_A : item_num × rank
      - gmf_A : item_num × rank
      - MLP LowRankLinear factors (A_l, B_l, bias for each of 3 layers)
      - Output head weights (small)
    B is NOT uploaded.
    """
    pf   = latent_dim
    bits = 0
    # item A only (no B)
    bits += item_num * rank * 2 * 32          # mlp_A + gmf_A
    # MLP 3 layers: each has (out×rank + in×rank + out) params
...
def _download_bits(item_num: int, latent_dim: int, rank: int) -> int:
    """
    Server broadcasts: A + B + MLP + output heads (full server model).
    """
    pf   = latent_dim
    bits = 0
    # item A + B for mlp and gmf
    bits += (item_num * rank + 2*pf * rank) * 2 * 32
    # MLP
    mlp_dims = [(4*pf, 2*pf), (2*pf, pf), (pf, pf//2)]
    for (in_f, out_f) in mlp_dims:
        bits += (out_f * rank + in_f * rank + out_f) * 32
...
# ── Federated NCF (paper-faithful CoLR) ──────────────────────────────────────
...
class FederatedNCF:
    def __init__(self,
        ...
        # N_u per client (interaction counts for weighted aggregation)
        ...
        # ── Comm cost report ──────────────────────────────────────────────────
        full_item_bits = self.item_num * 2 * latent_dim * 2 * 32   # full-rank baseline
        ul_bits        = _upload_bits(self.item_num, latent_dim, rank)
        dl_bits        = _download_bits(self.item_num, latent_dim, rank)
        saving_ul      = 100.0 * (1 - ul_bits / full_item_bits)
        ...
        print(f"{'='*60}")
        print(f"Method        : CoLR (paper-faithful, B fixed per round)")
        ...
        print(f"Upload/client : {ul_bits/8/1024:.2f} KB  "
              f"(A only, saving {saving_ul:.1f}% vs full-rank)")
        print(f"Download/cl.  : {dl_bits/8/1024:.2f} KB  (A + B + MLP)")
        print(f"Aggregation   : weighted FedAvg on A only  (B unchanged)")
    ...
    def _single_round(self, epoch: int,
                      dl_bits: int, ul_bits: int) -> tuple:
        """
        Paper CoLR per-round on each client:
          1. Receive A_global + B (server) + MLP
          2. Freeze B
          3. Train A_u + MLP locally
          4. Upload A_u + MLP (NOT B)
        """
        timings       = []
        ...
        for cid, client in bar:
            ...
            # ── Paper step: freeze B before local training ────────────────────
            client.ncf.freeze_item_B()
            ...
            # ── Collect A_u + shared weights for aggregation ──────────────────
    ...
    def _print_timing(self, epoch, timings, agg_time):
        ...
        print(f"\n{'='*60}")
        print(f"Epoch {epoch} Timing  [CoLR rank={self.rank}, B fixed]")
        print(f"{'='*60}")
    ...
    def train(self):
        ...
        print(f"Download : {dl_bits/8/1024:.2f} KB  |  "
              f"Upload : {ul_bits/8/1024:.2f} KB  (A only, B fixed)\n")

        for epoch in range(self.aggregation_epochs):
            # ── 1. Broadcast server weights (A + B + MLP) to all clients ──────
            for client in self.clients:
                client.ncf.to(self.device)
                client.ncf.load_server_weights(server_model)

            # ── 2. Local training (B frozen, only A + MLP trained) ────────────
            timings, client_mlp_As, client_gmf_As, client_shared = \
                self._single_round(epoch, dl_bits, ul_bits)

            # ── 3. Paper FedAvg: aggregate A only, B unchanged ────────────────
            t0 = time.time()
            server_model.aggregate_A(
