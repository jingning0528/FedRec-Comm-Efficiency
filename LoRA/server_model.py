...
class ServerNeuralCollaborativeFiltering(torch.nn.Module):
    def __init__(self, item_num, predictive_factor=32, lora_rank=8):
        super(ServerNeuralCollaborativeFiltering, self).__init__()
        self.lora_rank = lora_rank
        self.item_num = item_num
        self.predictive_factor = predictive_factor

        # Server stores A, B directly (same structure as client)
    ...
    def get_download_payload(self):
        """Return full state dict — keys already match client's load_server_weights."""
...
if __name__ == '__main__':
    ncf = ServerNeuralCollaborativeFiltering(100, 64, lora_rank=8)
    print(ncf)