...
def truncate_payload(payload: dict, rank: int) -> dict:
    ...
    for k, v in payload.items():
        if k in ('mlp_lora_A', 'gmf_lora_A'):          # (max_rank, emb_dim)
            out[k] = v[:rank, :]
        elif k in ('mlp_lora_B.weight', 'gmf_lora_B.weight'):  # (item_num, max_rank)
            out[k] = v[:, :rank]
        else:
            out[k] = v
    return out
...
def aggregate_rank_wise(client_payloads, client_weights, r_max):
    """
    Rank-wise weighted aggregation.
    For each rank dimension j, only aggregate clients whose rank > j.
    Padded zeros are never included — each dim averaged over real contributors only.
    """
    agg_A = {
        'mlp_lora_A': torch.zeros_like(client_payloads[0]['mlp_lora_A']),
        'gmf_lora_A': torch.zeros_like(client_payloads[0]['gmf_lora_A']),
    }
    agg_B = {
        'mlp_lora_B.weight': torch.zeros_like(client_payloads[0]['mlp_lora_B.weight']),
        'gmf_lora_B.weight': torch.zeros_like(client_payloads[0]['gmf_lora_B.weight']),
    }
    contrib_count = torch.zeros(r_max, dtype=torch.float32)

    for j in range(r_max):
        # Only clients whose actual rank covers dimension j
        valid = [i for i, p in enumerate(client_payloads) if p['lora_rank'].item() > j]
        if len(valid) == 0:
            continue

        weight_sum = sum(client_weights[i] for i in valid)

        for i in valid:
            w = client_weights[i] / weight_sum
            for A_key in agg_A:
                agg_A[A_key][j] += w * client_payloads[i][A_key][j].float()
            for B_key in agg_B:
                agg_B[B_key][:, j] += w * client_payloads[i][B_key][:, j].float()

        contrib_count[j] = len(valid)

    return agg_A, agg_B, contrib_count


def apply_dimension_weighting(agg_A, agg_B, contrib_count, num_clients, beta=0.5):
    """
    Scale each rank dimension j by confidence weight w_j = (m_j / N)^beta.
    Dims covered by many clients → w_j ≈ 1.0  (trusted, kept strong)
    Dims only covered by fast clients → w_j < 1.0  (shrunk proportionally)
    beta=0.5 is a soft square-root damping; increase for stronger shrinkage.
    """
    for A_key in agg_A:
        for j in range(agg_A[A_key].shape[0]):
            if contrib_count[j] == 0:
                continue
            w_j = (contrib_count[j] / num_clients) ** beta
            agg_A[A_key][j]    *= w_j
    for B_key in agg_B:
        for j in range(agg_B[B_key].shape[1]):
            if contrib_count[j] == 0:
                continue
            w_j = (contrib_count[j] / num_clients) ** beta
            agg_B[B_key][:, j] *= w_j
...
def federate(utils, beta=0.5):
    ...
    client_weights = [float(u['lora_rank'].item()) for u in updates]

    # Rank-wise weighted aggregation — dimension weighting is already implicit:
    # high-rank dims have fewer contributors → their weight_sum is smaller →
    # each contributor's w = rank_i / weight_sum is naturally larger.
    # No need to multiply again after aggregation (that causes exponential decay).
    agg_A, agg_B, contrib_count = aggregate_rank_wise(updates, client_weights, MAX_LORA_RANK)

    # Optional: EMA blend with previous server state for stability
    # instead of hard replacing, blend: new = alpha*agg + (1-alpha)*prev
    alpha = 0.8  # how much to trust new aggregation vs keep server state
    new_state = dict(prev_state)
    for k, v in agg_A.items():
        new_state[k] = alpha * v + (1 - alpha) * prev_state[k].float()
    for k, v in agg_B.items():
        new_state[k] = alpha * v + (1 - alpha) * prev_state[k].float()

    # Weighted FedAvg for shared layers
    ...
    for k in shared_keys:
        new_state[k] = sum(
            (client_weights[i] / weight_sum) * updates[i][k].float()
            for i in range(n)
        )
