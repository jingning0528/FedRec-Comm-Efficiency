...
def federate(utils):
    """FedAvg on A, B and shared layers — Low_rank algorithm (no SVD, no absorption)."""
...
class FederatedNCF:
    ...
    def _single_round(self, epoch: int, payload_size_bits: int) -> list:
        ...
        for cid, client in bar:
            ...
            # Save LoRA update (Low_rank algorithm — FedAvg on A, B)
    ...
    def _print_timing(self, epoch: int, timings: list, agg_time: float,
                      payload_size_bits: int, full_size_bits: int):
        ...
        print(f"\n{'='*60}")
        print(f"Epoch {epoch} Timing  [LoRA rank={self.lora_rank} | "
    ...
    def train(self):
        ...
        print(f"\n{'='*60}")
        print(f"Low_rank LoRA-FedAvg  [rank={self.lora_rank}, symmetric, no SVD]")
        ...
        for epoch in range(self.aggregation_epochs):
            # 1. Load server state and distribute to all clients
            server_model = ServerNeuralCollaborativeFiltering(
                item_num=item_num, predictive_factor=self.latent_dim, lora_rank=self.lora_rank)
            ...
            # 2. Local training + collect LoRA updates
            timings  = self._single_round(epoch, payload_size_bits)

            # 3. FedAvg on A, B (Low_rank algorithm — no absorption)
            agg_time = federate(self.utils)

            # 4. Timing summary
            self._print_timing(epoch, timings, agg_time, payload_size_bits, full_size_bits)
