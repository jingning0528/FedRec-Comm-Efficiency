...
class NeuralCollaborativeFiltering(torch.nn.Module):
    def __init__(self, user_num, item_num, predictive_factor=32, pretrained_item_embeddings=None):
    ...
    def initialize_user_embeddings_from_items(self, ui_matrix):
        """
        Initialize each user's embedding as the average of embeddings of items
        that user has interacted with.
        
        Args:
            ui_matrix: numpy array of shape (num_users, num_items), nonzero = interaction
        """
        import numpy as np
        with torch.no_grad():
            mlp_item_weight = self.mlp_item_embeddings.weight.data  # (num_items, emb_dim)
            gmf_item_weight = self.gmf_item_embeddings.weight.data  # (num_items, emb_dim)
            
            for user_idx in range(ui_matrix.shape[0]):
                interacted_items = np.where(ui_matrix[user_idx] != 0)[0]
                if len(interacted_items) > 0:
                    item_indices = torch.tensor(interacted_items, dtype=torch.long, device=mlp_item_weight.device)
                    # MLP user embedding = average of MLP item embeddings for interacted items
                    self.mlp_user_embeddings.weight.data[user_idx] = mlp_item_weight[item_indices].mean(dim=0)
                    # GMF user embedding = average of GMF item embeddings for interacted items
                    self.gmf_user_embeddings.weight.data[user_idx] = gmf_item_weight[item_indices].mean(dim=0)
                # else: keep the random initialization
