...
class NCFTrainer:
    ...
    Trainable params: lora_A, lora_B (item adapters) + user embeddings.
    Frozen params   : item base embeddings E0, MLP, output layers
                      (MLP/output ARE communicated but NOT locally updated —
                       they are received from the server each round).
    """

    def __init__(self, ui_matrix: np.ndarray, epochs: int, batch_size: int,
                 latent_dim: int = 32, lora_rank: int = 8,
                 device=None, global_user_offset: int = 0, eval_seed: int = 42):
        ...
        self.ncf = NeuralCollaborativeFiltering(
            ui_matrix.shape[0], ui_matrix.shape[1],
            predictive_factor=latent_dim, lora_rank=lora_rank,
        ).to(self.device)
        self.loader = MatrixLoader(self.ui_matrix)

        # Names of parameters that are locally trainable
        self._lora_param_names = {
            "mlp_lora_A", "mlp_lora_B", "gmf_lora_A", "gmf_lora_B",
            "mlp_user_embeddings.weight", "gmf_user_embeddings.weight",
        }
    ...
    def _train_batch(self, x, y, optimizer):
        pos_mask = (y > 0)
        neg_mask = ~pos_mask
    ...
    def train_model(self, optimizer, epochs=None):
        ...
        for epoch in range(epochs):
            ...
            while True:
                ...
                loss, y_ = self._train_batch(x, y, optimizer)
    ...
    def evaluate_standard(self, n_neg: int = 99, k: int = 10) -> dict:
        ...
        for local_u, test_item in self.test_items.items():
            ...
            hr, ng, loss = evaluate_user_standard(
                model=self.ncf, user_id=local_u, test_item=test_item,
                all_item_ids=self.all_item_ids, interacted=interacted,
                device=self.device, n_neg=n_neg, k=k,
            )
